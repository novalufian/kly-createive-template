/*PROTOTYPE CUSTOM FILTERING*/
    ...
    window.GAMLibrary = {
                ...
                if (scrollTop >= this.articleType[pageType].scroll) {
                    if (!this.timedBottomFrm) {
                        this.timedBottomFrm = googletag.defineSlot(this.dfpBottomframe, [
                            [320, 50],
                            [320, 100]
                        ], 'div-gpt-ad-bola-bottomfrm').addService(googletag.pubads()).setTargeting('pagingNum', '1');
                        setTimeout(() => {
                            if (pbjs) {
                                pbjs.setTargetingForGPTAsync(['div-gpt-ad-bola-bottomfrm']);
                ...
                if (scrollTop >= this.articleType[pageType].scroll) {
                    if (!this.timedFloatingPin) {
                        this.timedFloatingPin = googletag.defineSlot(this.dfpFloatingPin, [
                            [1, 1],
                            ['fluid']
                        ], 'div-gpt-ad-bola-floating-pin').addService(googletag.pubads()).setTargeting('pagingNum', '1');
        ...
        exposerInterscrollerStyle: function() {
            ...
            interscrollerStyle.textContent = "body{overflow: unset;} .interscroller-wrapper{min-height: 600px; display: block !important;} .interscroller{position: sticky !important; top: 160px;}";
    ...
    googletag.cmd.push(function() {
        ...
        if (GAMLibrary.topFrameFixedSize) {
            window.GAMLibrary.topframe = googletag.defineSlot(GAMLibrary.dfpTopframe, [1, 1], 'div-gpt-ad-bola-topfrm-oop').addService(googletag.pubads()).setTargeting('pagingNum', '1');
        } else {
            window.GAMLibrary.topframe = googletag.defineOutOfPageSlot(GAMLibrary.dfpTopframe, 'div-gpt-ad-bola-topfrm-oop').addService(googletag.pubads()).setTargeting('pagingNum', '1');
        }
    ...
    function showAds(adsContainer, size, adunit) {
        if (adsContainer && size && adunit) {
            ...
            var sizes = [],
                adunitPath = '/36504930/KLY/MOBILE/BOLA.NET/' + adunit,
                adSlot;
    ...
    var gptBolaStyle = document.createElement('style');
    gptBolaStyle.textContent = '#div-gpt-ad-bola-hl{-webkit-transition:margin .5s;transition:margin .5s;margin-top:0}.dfp-hl-sticky{position:fixed;top:0;min-height:50px;left:50%;margin-left:-160px!important;z-index:99999}';
    window.onload = function() {
        document.body.appendChild(gptBolaStyle);
    };
