...
window.GAMLibrary = {
    ...
    scrollBottomFrame: function() {
        this.scroll = function() {
            ...
            if (scrollTop >= this.articleType[pageType].scroll) {
                document.querySelector("#dfp-bframe-cont-transparent").style.setProperty("display", "block");
                if (!this.timedBottomFrm) {
                    this.timedBottomFrm = googletag.defineSlot(this.dfpBottomFrame, [
                        [320, 50],
                        [320, 100]
                    ], 'div-gpt-ad-merdeka-bottomfrm').addService(googletag.pubads()).setTargeting('pagingNum', '1');
                    setTimeout(() => {
                        if (pbjs) {
                            (typeof pbjs.setTargetingForGPTAsync == 'function') ? pbjs.setTargetingForGPTAsync(['div-gpt-ad-merdeka-bottomfrm']): '';
            ...
            if (scrollTop >= this.articleType[pageType].scroll) {
                if (!this.timedFloatingPin) {
                    this.timedFloatingPin = googletag.defineSlot(this.dfpFloatingPin, [
                        [1, 1],
                        ['fluid']
                    ], 'div-gpt-ad-merdeka-floating-pin').addService(googletag.pubads()).setTargeting('pagingNum', '1');
    ...
    exposerInterscroller: function(event) {
        ...
        if (_is300x250_ && _domId_.includes("exposer")) {
            ...
            interscrollerStyle.textContent = ".interscroller-wrapper{min-height: 600px;} .interscroller{position: sticky !important; top: 160px;}";
    ...
    brandSafetyChecker: function() {
        ...
        var siteContentObject = document.getElementsByClassName("pages");
        var siteContentText = "";

        if (siteContentObject.length) {
            /*change this acording to the site page layout*/
            var baca_juga_elements = siteContentObject[0].getElementsByClassName("pages-paragraph");
            for (var i in baca_juga_elements) {
                (typeof baca_juga_elements[i].innerText !== "undefined") ? (siteContentText += baca_juga_elements[i].innerText) : '';
    ...
    _SCFlyingCarpetStyle_: function() {
        ...
        _style_.textContent = `.box--banner div[id*="showcase"].flying-carpet{ position: relative; clear: both; overflow: hidden; clip-path: polygon(0px 0px, 100% 0px, 100% 100%, 0px 100%) !important; min-height: 270px; width: 100%; display: flex; justify-content: center; } .box--banner div[id*="showcase"].flying-carpet .ad-content { top: 50%; transform: translateY(-50%); position: fixed; z-index: 2; }`;
...
googletag.cmd.push(function() {
    ...
    if (GAMLibrary.topFrameFixedSize) {
        window.GAMLibrary.topframe = googletag.defineSlot(GAMLibrary.dfpTopframe, [1, 1], 'div-gpt-ad-merdeka-topfrm-oop').addService(googletag.pubads()).setTargeting('pagingNum', '1');
    } else {
        window.GAMLibrary.topframe = googletag.defineOutOfPageSlot(GAMLibrary.dfpTopframe, 'div-gpt-ad-merdeka-topfrm-oop').addService(googletag.pubads()).setTargeting('pagingNum', '1');
    }
...
function showAds(adsContainer, size, adunit) {
    if (adsContainer && size && adunit) {
        ...
        var sizes = [],
            adunitPath = '/36504930/KLY/MOBILE/MERDEKA.COM/' + adunit,
            adSlot;
        ...
        /*making adrequest*/
        googletag.cmd.push(function() {
            if (document.querySelector("#" + adsContainer).parentElement.parentElement.parentElement.id == "articles-2" && adunit == "HEADLINE") { GAMLibrary.dfpPageNum = 1; }
